...
//       const scale = Math.max(Math.min(w / 1000, h / 600), 0.45);
...
//     // --- GAME ENGAGEMENT & DIFFICULTY FORMULAS ---
//     const getDifficultyLevel = () => scoreRef.current / 100;

//     const getTargetSpeed = (level: number) => {
//       const baseSpeed = 0.35 + 0.08 * Math.sqrt(level);
//       const streakSurge = streakRef.current >= 3 ? 1.2 : 1.0;
...
//     const getWindStrength = (level: number) => {
//       if (scoreRef.current < 300) return 0; // Gentle onboarding: No wind initially
//       const baseWind = 3.5 * Math.sqrt(level - 3);
//       return streakRef.current >= 5 ? baseWind * 1.3 : baseWind;
//     };

//     const getBullseyeRadius = (level: number) => {
//       return Math.max(6, BASE_BULLSEYE_RADIUS - Math.min(level * 0.1, 5));
//     };
...
//       if (level > 10) {
//         // Complex Figure-8 path for masters
//         return BASE_TY + Math.sin(timeElapsed * speed) * amp;
//       }
//       return BASE_TY + Math.sin(timeElapsed * speed) * amp;
//     };

//     const getTargetXOffset = () => {
//       const level = getDifficultyLevel();
//       if (level > 10) {
//         const speed = getTargetSpeed(level);
...
//       const windMax = getWindStrength(getDifficultyLevel());
...
//       const GROUND_Y = Py + 35;
//       const groundGrad = ctx.createLinearGradient(0, GROUND_Y, 0, HEIGHT + 400);
//       groundGrad.addColorStop(0, t.ground[0]);
//       groundGrad.addColorStop(1, t.ground[1]);
//       ctx.fillStyle = groundGrad;
//       ctx.fillRect(-2000, GROUND_Y, 5000, HEIGHT + 1000);

//       if (targetShake > 0) targetShake = Math.max(0, targetShake - dt * 4);
//       const shakeX = (Math.random() - 0.5) * targetShake * 15;

//       const dynamicLevel = getDifficultyLevel();
//       const currentTx = Tx + getTargetXOffset();
//       const Ty = getTargetY();
...
//       ctx.fillRect(currentTx - 12, Ty, 16, GROUND_Y - Ty);
//       ctx.fillStyle = t.targetStand[1];
//       ctx.fillRect(currentTx - 16, Ty, 10, GROUND_Y - Ty);
...
//         ctx.fillStyle = grad;
//         ctx.beginPath();
//         ctx.arc(currentTx, Ty, r, 0, Math.PI * 2);
//         ctx.fill();
//       };
...
//       // --- AIM LINE & ANTI-FRUSTRATION PITY ASSIST ---
//       if (isDragging) {
//         ctx.beginPath();
...
//         // Catch-Up Trajectory Arc (if player missed 3 times in a row)
//         if (consecutiveMissesRef.current >= 3) {
//           ctx.beginPath();
//           let simX = handX,
//             simY = handY;
//           let simVx =
//             Math.min(dragDist, 250) * powerMultiplier * Math.cos(displayAngle);
//           let simVy =
//             Math.min(dragDist, 250) * powerMultiplier * Math.sin(displayAngle);
...
//         ctx.moveTo(-40, 0);
//         ctx.lineTo(20, 0);
//         ctx.strokeStyle = streakRef.current >= 3 ? "#F59E0B" : t.targetStand[0];
//         ctx.lineWidth = streakRef.current >= 3 ? 4.5 : 3.5;
//         ctx.stroke();
//         ctx.beginPath();
//         ctx.moveTo(25, 0);
//         ctx.lineTo(5, -7);
//         ctx.lineTo(5, 7);
//         ctx.fillStyle = streakRef.current >= 3 ? "#EF4444" : t.sunGlow;
//         ctx.fill();
//       };
...
//           targetShake = 1.0;
//           streakRef.current += 1;
//           consecutiveMissesRef.current = 0; // Reset pity counter
